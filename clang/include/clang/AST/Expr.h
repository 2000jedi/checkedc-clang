...
public:
  ArraySubscriptExpr(Expr *lhs, Expr *rhs, QualType t, ExprValueKind VK,
      ...
      : Expr(ArraySubscriptExprClass, t, VK, OK), Bounds(nullptr) {
    SubExprs[LHS] = lhs;
    SubExprs[RHS] = rhs;
...
public:
  InvariantClause(SourceLocation Loc, Expr *Cond, ArrayRef<VarDecl *> Vars) : Loc(Loc), Cond(Cond), Vars(Vars) {}
